...
def random_search(clf, X_train, y_train, clf_type="xgb"):
    ...
    rnd_search = RandomizedSearchCV(clf, param_distributions=param_grid, 
                                    n_iter=10, cv=15, verbose=2, n_jobs=-1,
...
if __name__ == "__main__":
    ...
    if test_evaluation["Accuracy"] < 0.95:
        best_params, best_score, cv_results, clf = random_search(clf, X_train, y_train, clf_type="rf")
        cv_results_df = pd.DataFrame(cv_results)

        final_evaluation = evaluate(clf, X_test, y_test)
        save_metadata(clf, df, metadata_path, 
                      best_params, 

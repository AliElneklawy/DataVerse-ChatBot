...
class CrawlerProgress:
    ...
    @staticmethod
    def update_progress(crawl_id: str, status: str = None, total_urls: int = None, 
                       crawled_urls: int = None, current_url: str = None, 
                       error: str = None, log_message: str = None) -> None:
        ...
        with _lock:
            ...
            if progress['total_urls'] > 0 and progress['crawled_urls'] > 0:
                ...
                urls_per_second = progress['crawled_urls'] / elapsed if elapsed > 0 else 0
        ...
        with _lock:
            if crawl_id in _crawler_progress:
                ...
                if progress['total_urls'] > 0:
                    progress['percentage'] = (progress['crawled_urls'] / progress['total_urls']) * 100
                else:
                    progress['percentage'] = 0
                    
                # Check for stalled crawl (no updates in 60 seconds)
                if progress['status'] not in ['completed', 'error'] and time.time() - progress['last_update'] > 60:
    ...
    def complete_progress(crawl_id: str, success: bool = True) -> None:
        ...
        with _lock:
            if crawl_id in _crawler_progress:
                _crawler_progress[crawl_id]['status'] = 'completed' if success else 'error'
                ...
                if success and _crawler_progress[crawl_id]['total_urls'] > 0:
                    _crawler_progress[crawl_id]['crawled_urls'] = _crawler_progress[crawl_id]['total_urls']
...
class AdminUtils:
    ...
    def get_system_stats(self, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_ops.db_path) as conn:
                ...
                stats['model_usage'] = [{'model': row[0], 'count': row[1]} for row in cursor.fetchall()]
    ...
    def generate_usage_report(self, start_date: datetime, end_date: datetime) -> BytesIO:
                ...
                with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                    ...
                    summary = {
                        ...
                        'Value': [
                            ...
                            cost_df['input_tokens'].sum() + cost_df['output_tokens'].sum(),
                    ...
                    cost_per_model = cost_df.groupby('model_used').agg({
                        'input_tokens': 'sum',
                        'output_tokens': 'sum',
                        'request_cost': 'sum'
                    }).reset_index()
                    cost_per_model.to_excel(writer, sheet_name='Cost Per Model', index=False)
        ...
        try:
            with sqlite3.connect(self.db_ops.db_path) as conn:
                ...
                if not model_data.empty:
                    ...
                    plt.pie(model_data['count'], labels=model_data['model_used'], autopct='%1.1f%%')
    ...
    def set_email_configuration(self, email: str, enable_notifications: bool) -> bool:
        ...
        try:
            ...
            self.update_system_config('email_notifications_enabled', str(enable_notifications).lower())
    ...
    async def test_rag_system(self, query: str, content_path: Path, 
                              model_name: str, provider: str, 
                              chunking_type: str, rerank: bool) -> Tuple[bool, str, float]:
        ...
        try:
            if provider == 'cohere':
                ...
                rag = CohereRAG(content_path, INDEXES_DIR, 
                               model_name=model_name,
                               chunking_type=chunking_type,
                               rerank=rerank)
            elif provider == 'claude':
                from ..rag.claude_rag import ClaudeRAG
                rag = ClaudeRAG(content_path, INDEXES_DIR, 
                               model_name=model_name,
                               chunking_type=chunking_type,
                               rerank=rerank)
            elif provider == 'gemini':
                from ..rag.gemini_rag import GeminiRAG
                rag = GeminiRAG(content_path, INDEXES_DIR, 
                               model_name=model_name,
                               chunking_type=chunking_type,
                               rerank=rerank)
            elif provider == 'grok':
                from ..rag.grok_rag import GrokRAG
                rag = GrokRAG(content_path, INDEXES_DIR, 
                             model_name=model_name,
                             chunking_type=chunking_type,
                             rerank=rerank)
            elif provider == 'openai':
                from ..rag.openai_rag import OpenAIRAG
                rag = OpenAIRAG(content_path, INDEXES_DIR, 
                               model_name=model_name,
                               chunking_type=chunking_type,
                               rerank=rerank)
            elif provider == 'mistral':
                from ..rag.mistral_rag import MistralRAG
                rag = MistralRAG(content_path, INDEXES_DIR, 
                                model_name=model_name,
                                chunking_type=chunking_type,
                                rerank=rerank)
            elif provider == 'deepseek':
                from ..rag.deepseek_rag import DeepseekRAG
                rag = DeepseekRAG(content_path, INDEXES_DIR, 
                                 model_name=model_name,
                                 chunking_type=chunking_type,
                                 rerank=rerank)
            else:

...
class ChatHistoryMonitor(BaseMonitorService):
    ...
    def _retrieve_and_email(self):
        ...
        if not created_files:
            logger.warning(f"{self.__class__.__name__}: Failed to create any history files.")
            return
        ...
        if hasattr(self.email_service, 'send_email_with_attachments'):
            ...
            logger.info(f"{self.__class__.__name__}: Found previous conversations. Sending email with JSON only...")
            subject = "Chat History for Your Chatbot"
            
            json_file = next((f for f in created_files if f.suffix.lower() == '.json'), None)
            if json_file:
    ...
    def _create_pdf(self, data, output_path):
        ...
        try:
            try:
                pdfmetrics.registerFont(TTFont('ArabicFont', FONTS_DIR / "Amiri-Regular.ttf"))
                pdfmetrics.registerFont(TTFont('EnglishFont', FONTS_DIR / "Helvetica.ttf"))
            except Exception as e:
            ...
            elements = []
            table_data = self._prepare_pdf_table_data(data, english_style, arabic_style, styles)
        ...
        for user_entry in data:
            ...
            if interactions:
                ...
                row = [
                    ...
                    Paragraph(self._prepare_arabic_text(first_interaction.get('user', '')), arabic_style),
                    Paragraph(self._prepare_arabic_text(first_interaction.get('assistant', '')), arabic_style)
                ]
            ...
            for interaction in interactions[1:]:
                row = [
                    ...
                    Paragraph(self._prepare_arabic_text(interaction.get('user', '')), arabic_style),
                    Paragraph(self._prepare_arabic_text(interaction.get('assistant', '')), arabic_style)
                ]
                table_data.append(row)

...
class DatabaseOps:
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                ...
                (
                    ...
                    output_tokens,
                    input_tokens,
                    cost_per_input_token / 10**6,
                    output_tokens,
                    cost_per_output_token / 10**6,
                ),
    ...
    def get_monitored_resp(self) -> list[tuple[str, str]]:
        day_ago = datetime.now() - timedelta(
            hours=24
...
class EmailService:
    def __init__(self):
        self._subscribers = []
        self.sender_email = "ali.mostafa.elneklawy@gmail.com"
    ...
    def _format_email_content(self, unknowns):
        ...
        email_content += (
            "<table border='1'><tr><th>ID</th><th>Question</th><th>Response</th></tr>"
        )

        for i, (question, answer) in enumerate(unknowns, 1):
            truncated_answer = (answer[:100] + "...") if len(answer) > 100 else answer
            email_content += (
                f"<tr><td>{i}</td><td>{question}</td><td>{truncated_answer}</td></tr>"
            )

        email_content += "</table>"
        return email_content

    def _send_without_attachment(
        self, message: MIMEMultipart, unknowns: list[tuple[str, str]]
    ):
        """
    ...
    def _add_file_attachment(
        self, message: MIMEMultipart, file_path: str, content_type=None
    ):
        """
    ...
    def send_email_with_attachments(
        self, subject: str, message_body: str, file_paths: list[str] = None
    ):
        """
        ...
        try:
            with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
                ...
                server.sendmail(
                    self.sender_email, self._receiver_email, message.as_string()
                )
            logger.info(
                f"Email sent to {self._receiver_email} with subject: {subject} and {len(file_paths) if file_paths else 0} attachments"
            )
        except Exception as e:
            logger.error(f"Failed to send email: {e}")

    def _send_with_attachment(
        self, message: MIMEMultipart, json_data: list[dict[str, Any]], filename: str
    ):
        """
        ...
        """
        text_part = MIMEText(
            "Please find the attached JSON file of your conversations.\n\nBest Regards,\n",
            "plain",
        )
        message.attach(text_part)

...
class MistralRAG(BaseRAG):
    ...
    def _create_messages(self, query: str, context: str, user_id: str) -> list:
        ...
        system_prompt = self._generate_system_prompt(
            query, user_id, context, include_prev_conv=False, include_query=False
        )
        ...
        history = self.db.get_chat_history(
            user_id
        )  # returns "No previous conversations." for new users
    ...
    async def get_response(self, query: str, user_id: str) -> str:
        ...
        try:
            response = await self.mistral_client.chat.complete_async(
                model=self.model_name, messages=messages, temperature=RAGConfig.TEMPERATURE
            )

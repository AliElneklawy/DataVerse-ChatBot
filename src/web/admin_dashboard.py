...
def dashboard():
    ...
    with sqlite3.connect(db_ops.db_path) as conn:
        ...
        cursor = conn.execute(
            "SELECT COUNT(*) FROM chat_history WHERE timestamp >= ?", (time_window,)
        )
    ...
    for index_dir in INDEXES_DIR.glob("*"):
        if index_dir.is_dir() and (index_dir / "index.faiss").exists():
            index_size = (
                sum(f.stat().st_size for f in index_dir.glob("**/*") if f.is_file())
                / 1024
            )  # KB
            index_modified = datetime.fromtimestamp(
                max(f.stat().st_mtime for f in index_dir.glob("**/*") if f.is_file())
            )
            indexes.append(
                {
                    "name": index_dir.name,
...
def user_detail(user_id):
    ...
    with sqlite3.connect(db_ops.db_path) as conn:
        ...
        if interactions:
            user_history = [{"user_id": user_id, "interactions": interactions}]
...
def view_training_file(filename):
    ...
    if file_extension in binary_formats:
        flash(
            "Binary files cannot be viewed directly. Please index the file to extract content.",
            "warning",
        )
        return redirect(url_for("content"))
    ...
    try:
        ...
        flash(
            "This file appears to be binary and cannot be viewed directly.", "warning"
        )
        return redirect(url_for("content"))
...
def index_training_file(filename):
    ...
    with sqlite3.connect(app.config["ADMIN_DB_PATH"]) as conn:
        cursor = conn.execute(
            "SELECT config_value FROM system_config WHERE config_key = 'default_chunking_type'"
        )
        row = cursor.fetchone()
    ...
    try:
        ...
        if file_extension in binary_formats:
            ...
            logger.info(
                f"Processing binary file: {filename} with extension {file_extension}"
            )
            ...
            loader = FileLoader(
                str(file_path), str(content_file_path), client="langchain"
            )
            ...
            if content_file_path.exists():
                content_hash = (
                    hashlib.sha256(str(content_file_path).encode("utf-8")).hexdigest(),
                    16,
                )[0][:15]
            ...
            if existing_index_path:
                ...
                flash(
                    f"Content from {filename} extracted and added to existing knowledge base",
                    "success",
                )
            else:
                ...
                flash(
                    f"Content from {filename} extracted and indexed successfully",
                    "success",
                )
        else:
...
def upload_content():
    ...
    if file_extension not in allowed_extensions:
        flash(
            f"File type {file_extension} is not supported. Supported types: {', '.join(allowed_extensions)}",
            "danger",
        )
        return redirect(url_for("content"))
    ...
    if file:
        ...
        if file_extension in [
            ...
            flash(
                f'File {filename} uploaded successfully. Click "Index" to extract and index the content.',
                "success",
            )
        else:
            flash(
                f'File {filename} uploaded successfully. Click "Index" to index the content.',
                "success",
            )
...
def crawl_website():
    ...
    try:
            ...
            try:
                ...
                content_path = loop.run_until_complete(
                    crawler.extract_content(
                        url, webpage_only=False, max_depth=max_depth
                    )
                )
                loop.close()
        ...
        return jsonify(
            {"success": False, "message": f"Error starting crawler: {str(e)}"}
        )
...
def crawler_progress(crawl_id):
    ...
    if progress:
        ...
        return jsonify({"success": False, "message": "Crawl session not found"}), 404
...
def view_content(filename):
    ...
    return render_template(
        "admin/view_content.html", filename=filename, content=content
    )
...
def system_email():
    ...
    with sqlite3.connect(app.config["ADMIN_DB_PATH"]) as conn:
        ...
        cursor = conn.execute(
            "SELECT COUNT(*) FROM system_config WHERE config_key = 'email_address'"
        )
        if cursor.fetchone()[0] > 0:
        ...
        conn.execute(
            "UPDATE system_config SET config_value = ?, updated_at = ? WHERE config_key = ?",
            (
                str(enable_notifications).lower(),
                datetime.now(),
...
def index_content(filename):
    ...
    try:
        ...
        if file_extension in binary_formats:
            ...
            loader = FileLoader(
                str(file_path), str(content_file_path), client="docling"
            )
            documents = loader.extract_from_file()
                ...
                if existing_index.is_dir():
                    ...
                    try:
                        ...
                        if content_file_path.exists():
                            content_hash = (
                                hashlib.sha256(
                                    str(content_file_path).encode("utf-8")
                                ).hexdigest(),
                                16,
                            )[0][:15]
                            index_name = f"index_{content_hash}.faiss"
            ...
            # Create the RAG instance for indexing
            rag = CohereRAG(
                file_path, INDEXES_DIR, chunking_type=chunking_type, rerank=False
            )
...
def system():
    ...
    with sqlite3.connect(app.config["ADMIN_DB_PATH"]) as conn:
        ...
        cursor = conn.execute(
            "SELECT config_value FROM system_config WHERE config_key = 'email_notifications_enabled'"
        )
        row = cursor.fetchone()
        ...
        # Get email address
        cursor = conn.execute(
            "SELECT config_value FROM system_config WHERE config_key = 'email_address'"
        )
        row = cursor.fetchone()
    ...
    with sqlite3.connect(db_ops.db_path) as conn:
        ...
        for user_id, timestamp, question, answer, model, embedder in all_interactions:
            if user_id not in history_by_user:
                history_by_user[user_id] = {"user_id": user_id, "interactions": []}
...
def history_stats():
    ...
    return jsonify({"daily_queries": daily_queries, "model_usage": model_usage})
...
def test_content():
    ...
    if not content_path.exists():
        return jsonify(
            {"success": False, "error": f"Content file not found: {filename}"}
        )
    ...
    return jsonify({"success": success, "response": response, "time_taken": time_taken})
...
def export_user_data(user_id):
    ...
    if format_type == "json":
        ...
        response.headers["Content-Disposition"] = (
            f"attachment; filename={user_id}_export.json"
        )
        return response
        ...
        response = app.response_class(
            ...
            mimetype="text/csv",
            headers={
                "Content-Disposition": f"attachment; filename={user_id}_export.csv"
            },
        )
        return response
...
def export_history():
    ...
    db_ops = DatabaseOps()
    history = db_ops.get_chat_history(
        full_history=True,
        last_n_hours=None if timeframe == "all" else int(timeframe.rstrip("dhm")),
    )
    ...
    if format_type == "json":
        ...
        response.headers["Content-Disposition"] = (
            f"attachment; filename=chat_history_{timestamp}.json"
        )
        return response
        ...
        response = app.response_class(
            ...
            mimetype="text/csv",
            headers={
                "Content-Disposition": f"attachment; filename=chat_history_{timestamp}.csv"
            },
        )
        return response
...
def backup_system():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Create zip file
            backup_filename = (
                f"rag_system_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
            )
...
def system_status():
    ...
    try:
        ...
        return jsonify({"status": [False, False, False, False, False], "error": str(e)})

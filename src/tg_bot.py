...
class TelegramBot:
    ...
    async def fetch_content(
        self, link, domain_name, max_depth=None, file_path=None, webpage_only=True
    ):
        if link:
    ...
    async def _init_rag_system(self):
        ...
        content_path = await self.fetch_content(
            self.link, domain_name, webpage_only=False
        )
        self.rag = CohereRAG(content_path, INDEXES_DIR, chunking_type="recursive")
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        user_id = user.id
        msg = (
            "مرحبًا بك في بوت الدعم الخاص بشركة بيسايتك، وجهتك الأولى لعالم التقنية والذكاء الإصطناعي ودعم الأعمال.\n"
    ...
    async def transcribe(self, audio_buffer: io.BytesIO):
        transcription = await self.client.audio.transcriptions.create(
            model="whisper-1", file=audio_buffer, response_format="text"
        )
    ...
    async def add_content(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await msg.edit_text("I added the new content successfully!")
    ...
    async def handle_question(
        ...
        try:
            ...
            await update.message.reply_text(
                "عذرًا، حدث خطأ أثناء معالجة سؤالك. حاول مرة أخرى لاحقًا."
            )
    ...
    async def cancel_conversation(
        ...
        await update.message.reply_text(
            "تم إلغاء المحادثة. يمكنك البدء من جديد باستخدام /start."
        )
    ...
    async def set_email(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.args:
            ...
            await update.message.reply_text(
                f"Email unset. You won't receive updates about your bot's responses."
            )
            return
        ...
        if not self._is_valid_email(new_email):
            await update.message.reply_text(
                "The email is invalid. Please double-check the email that you provided."
            )
            return
    ...
    async def broadcast(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await context.bot.send_message(
                chat_id=user_id, text=f"Something went wrong ({chat_id}): {e}"
            )
            logger.error(f"Something went wrong: {e}")
        else:
            await context.bot.send_message(
                chat_id=user_id, text="Messages broadcasted successfully."
            )
